...
class OdooAwait {
  ...
  connect() {
    ...
    return new Promise((resolve, reject) => {
      this.client.methodCall('authenticate', params, function (err, uid) {
        if (err) {
          reject(err);
          return;
        }
  ...
  execute_kw(model, method, params) {
    ...
    return new Promise((resolve, reject) => {
      this.client.methodCall('execute_kw', params, function (err, resp) {
        if (err) {
          reject(err);
          return;
        }
      ...
      if (val && val.action) {
        switch (val.action) {
          case 'create': // creates records and adds them to the set
            if (!val.value) {
              throw new Error(
                "'create' action requires a value object or an array of value objects"
              );
            }
            params[key] = Array.isArray(val.value)
              ? val.value.map((value) => {
                  return [0, 0, value];
                })
              : [[0, 0, val.value]];
          ...
          case 'update': // updates an existing record with new values
            if (!val.id || !val.value) {
              throw new Error("'update' action requires both an ID number and a value object");
            }
          ...
          case 'add': // adds an existing record to the set
            if (!val.id) {
              throw new Error(
                "'add' action requires an ID or array of ID's to add to add to the set"
              );
            }
            params[key] = Array.isArray(val.id)
              ? val.id.map((id) => {
                  return [4, id, 0];
                })
              : [[4, val.id, 0]];
          ...
          case 'remove': // removes the record of ID from the set. Does not delete it from the database
            if (!val.id) {
              throw new Error(
                "'remove' action requires an ID or array of ID's to remove from the set"
              );
            }
            params[key] = Array.isArray(val.id)
              ? val.id.map((id) => {
          ...
          case 'delete': // removes the record of ID from the set, and deletes it from the database.
            if (!val.id) {
              throw new Error("'delete' action requires an ID or array of ID's to delete");
            }
            params[key] = Array.isArray(val.id)
              ? val.id.map((id) => {
                  return [2, id, 0];
          ...
          case 'replace': // replaces all existing records in the set by the ids list, equivalent to using 'clear' and then 'add' on every id in the list
            if (!val.id) {
              throw new Error("'replace' action requires an ID or array of ID's to add to the set");
            }
  ...
  async searchRead(
    ...
    domain,
    fields = [],
    opts = {offset: 0, limit: 0, order: '', context: {}}
  ) {
    let domainArray = [];
    if (domain) {
      ...
      if (
        ...
        Array.isArray(domain) &&
        (Array.isArray(domain[0]) || ['&', '|', '!'].includes(domain[0]))
      ) {
        domainArray = domain;
      } else {
  ...
  async getFields(model, attributes = []) {
    return await this.execute_kw(model, 'fields_get', [
      [],
      {
  ...
  async getCookie() {
    return this.client.getCookie('session');
  }
}
